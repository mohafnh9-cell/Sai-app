...
export const MITIGATION_TEMPLATES: readonly MitigationTemplate[] = [
  ...
  },
  {
    ...
    protectionSignals: [
      ...
      "rejected invalid/missing signatures",
      "webhook rejected invalid",
      "401",
      "403",
      "signature checked",
    ],
  ...
  },
  {
    ...
    exploitSignals: [
      ...
      "duplicate operation",
      "duplicateOperation",
      "extra effect observed",
      "extraEffectObserved",
    ],
    protectionSignals: ["duplicate prevented", "cached idempotent", "sameResult", "idempotency enforced"],
    defaultSeverity: "high",
  },
  {
  ...
  },
  {
    ...
    rootCause: "Authentication or sensitive endpoints accept unlimited rapid requests.",
    recommendedProtection: "Enforce per-IP and per-identity rate limits with 429 responses.",
    implementationSteps: [
      "Add rate limiting middleware to login, signup, and password reset routes.",
      "Return Retry-After headers when throttling.",
      "Monitor blocked attempts for abuse patterns.",
    ],
    likelyAffectedFiles: ["app/api/**/route.ts", "middleware.ts", "server/**/rate-limit/**"],
    implementationRisk: "low",
    estimatedLoc: 30,
    rollbackGuidance: "Relax limits temporarily if legitimate users are blocked.",
    residualRisk: "Distributed attacks may require additional WAF rules.",
    exploitSignals: ["100 rapid requests", "no rate limiting", "requests accepted: 100", "not throttled"],
    protectionSignals: ["429", "rate limit enforced", "retry-after", "requests blocked"],
    defaultSeverity: "high",
  },
  {
    adapterId: "mass-assignment-probe",
    rootCause: "Server accepts privileged fields from client-controlled request bodies.",
    recommendedProtection: "Use explicit allowlists for writable fields on every mutating endpoint.",
    implementationSteps: [
      "Define DTO/schema with only safe writable fields.",
      "Strip or reject role, admin, and organizationId from client input.",
      "Add regression test for privileged field tampering.",
    ],
    likelyAffectedFiles: ["app/api/**/route.ts", "server/**/schemas/**"],
    implementationRisk: "medium",
    estimatedLoc: 35,
    rollbackGuidance: "Revert field stripping if legitimate admin updates break.",
    residualRisk: "Nested objects may still carry privileged keys.",
    exploitSignals: ["mass assignment", "privileged field accepted", "privilegedFieldAccepted\":true", "role\":\"admin\""],
    protectionSignals: [
  ...
  },
  {
    ...
    rootCause: "Admin-only resources are accessible to non-admin identities.",
    recommendedProtection: "Require server-side role checks before admin handler logic.",
    implementationSteps: [
      "Add requireAdmin or equivalent guard to admin routes.",
      "Verify role from session, not from client headers or body.",
      "Add tests for standard user accessing admin endpoints.",
    ],
    likelyAffectedFiles: ["app/api/admin/**", "server/**/auth/**"],
    implementationRisk: "medium",
    estimatedLoc: 30,
    rollbackGuidance: "Document any intentionally public admin endpoints.",
    residualRisk: "Parallel admin routes may remain unguarded.",
    exploitSignals: ["privilege escalation", "admin-only resource", "admin access: true"],
    protectionSignals: ["403", "admin resource denied", "forbidden"],
    defaultSeverity: "critical",
  },
  {
    adapterId: "security-headers-probe",
    rootCause: "Responses lack baseline security headers such as CSP and HSTS.",
    recommendedProtection: "Set Content-Security-Policy, Strict-Transport-Security, and related headers.",
    implementationSteps: [
      "Configure headers in Next.js config or edge middleware.",
      "Start with report-only CSP if needed, then enforce.",
      "Verify headers on all production responses.",
    ],
    likelyAffectedFiles: ["next.config.ts", "middleware.ts"],
    implementationRisk: "low",
    estimatedLoc: 20,
    rollbackGuidance: "Use report-only mode while tuning CSP.",
    residualRisk: "Third-party scripts may require CSP exceptions.",
    exploitSignals: ["missing csp", "missing hsts", "missing content-security-policy", "missing security headers"],
    protectionSignals: ["security headers present", "content-security-policy"],
    defaultSeverity: "medium",
  },
  {
    adapterId: "injection-probe-safe",
    rootCause: "User input may be reflected or executed without sanitization.",
    recommendedProtection: "Use parameterized queries, output encoding, and strict input validation.",
    implementationSteps: [
      "Replace string concatenation in queries with parameterized APIs.",
      "Encode output in HTML contexts.",
      "Reject unexpected characters at validation layer.",
    ],
    likelyAffectedFiles: ["app/api/**", "server/**/db/**"],
    implementationRisk: "medium",
    estimatedLoc: 40,
    rollbackGuidance: "Revert sanitization if legitimate input patterns break.",
    residualRisk: "Secondary injection paths may remain.",
    exploitSignals: ["payload reflected", "sql probe", "injection", "' or '1'='1"],
    protectionSignals: ["sanitized", "rejected", "400", "payload reflected: false"],
    defaultSeverity: "high",
  },
  {
    adapterId: "ssrf-probe-safe",
    rootCause: "Server-side fetch uses user-controlled URLs without outbound restrictions.",
    recommendedProtection: "Allowlist outbound hosts and block private/metadata IP ranges.",
    implementationSteps: [
      "Validate URLs against an allowlist before fetching.",
      "Block RFC1918, link-local, and metadata endpoints.",
      "Use a dedicated egress proxy for outbound requests.",
    ],
    likelyAffectedFiles: ["server/**/fetch/**", "app/api/**/route.ts"],
    implementationRisk: "medium",
    estimatedLoc: 45,
    rollbackGuidance: "Expand allowlist for required integrations.",
    residualRisk: "DNS rebinding may need additional controls.",
    exploitSignals: ["internalFetch\":true", "internal fetch: true", "169.254.169.254", "metadata url"],
    protectionSignals: [
      "blocked by allowlist",
      "internalFetch\":false",
      "internal fetch: false",
  ...
  },
  {
    ...
    rootCause: "CORS policy allows overly permissive cross-origin access.",
    recommendedProtection: "Restrict Access-Control-Allow-Origin to explicit trusted origins.",
    implementationSteps: [
      "Remove wildcard origin when credentials are enabled.",
      "Maintain an explicit allowlist of frontend origins.",
      "Validate Origin header server-side.",
    ],
    likelyAffectedFiles: ["middleware.ts", "next.config.ts", "app/api/**/route.ts"],
    implementationRisk: "low",
    estimatedLoc: 25,
    rollbackGuidance: "Add missing origins to allowlist rather than reverting to wildcard.",
    residualRisk: "Subdomain takeover can still abuse trusted origins.",
    exploitSignals: ["allow-origin: *", "wildcard with credentials", "corsWildcardWithCredentials"],
    protectionSignals: ["cors restricted", "explicit allowlist", "corsRestricted"],
    defaultSeverity: "medium",
  },
] as const;
