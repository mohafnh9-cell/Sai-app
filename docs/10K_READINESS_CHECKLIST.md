...
- [ ] **Rate limiting distribuido.** Código listo (`server/http/rate-limit.ts` ahora usa `@upstash/ratelimit` + `@upstash/redis` con sliding window cuando `UPSTASH_REDIS_REST_URL`/`UPSTASH_REDIS_REST_TOKEN` están seteadas; fallback automático a la ventana en memoria existente si no lo están, para no romper tests/dev local). Los ~60 call sites de `enforceRateLimit` se migraron a `await` (ahora es async). Typecheck, lint, `npm run build` y suite completa (309 archivos / 1782 tests) en verde. **Pendiente:** crear la base de datos en Upstash y setear las credenciales en Vercel — sin eso sigue corriendo en modo memoria-por-proceso, no distribuido de verdad.
