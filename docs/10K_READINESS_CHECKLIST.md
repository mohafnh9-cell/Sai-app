...
- [ ] **Validar Cohorte 0** (equipo + 3–5 founders amigos) contra sus propias métricas de salida: setup MCP <60s, veredicto P95 <2min, 0 incidentes P0. Ver `docs/roadmap/05-beta-milestones.md`. **Parcial — 2026-08-25**: las métricas medibles con datos reales de producción (`yuuytktcxxspoobsjcci`, últimos 30 días, 258 scans) quedan validadas: veredicto P95 = 14.2s (automatic) / 64.3s (manual), muy por debajo de 2 min; 5 fallos de 258 (1.9%), todos timeouts de red/GitHub o rate limits — ninguno es incidente de seguridad P0. Dos outliers de duración (14.8h y 57min) marcados `completed` mucho después de lo esperado — parecen recuperación de jobs atascados, no espera real del usuario; sin investigar a fondo. **Cerrado 2026-08-26**: telemetría de "setup MCP <60s" ya existe — `mcp_api_keys.first_used_at` (migración 055, aplicada en prod por el usuario) se setea una sola vez en el primer uso real de la key, sin pisarse en usos posteriores (a diferencia de `last_used_at`). P50/P95 de duración de setup + conteo de keys nunca usadas ya visibles en `/admin`. Verificado: typecheck, `npm run build`, suite completa (309/311 archivos) en verde; deploy a prod confirmado sano (`/`, `/.well-known/...`, `/admin`, `/api/mcp/keys` responden como se espera). Aún sin datos suficientes para reportar el P50/P95 real (keys nuevas desde el deploy). Sigue pendiente: **reclutar a los 3–5 founders amigos es un paso humano/de negocio, no ejecutable por el agente**.
