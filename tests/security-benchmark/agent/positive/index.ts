...
export const AGENT_POSITIVE_CASES: BenchmarkCase[] = [
  ...
  {
    id: "bash.credential.ssh-key-read-positive-01",
    ...
    source: "benchmark-new",
    description:
      "FIXED in the Detection Accuracy Hardening V1 pass: an agent tool's shell handler reads an SSH private key. Previously NOT_COVERED because isRelevantValue() (features/security-analysis/agent-action/discover.ts) pre-filtered every extracted string through a fixed keyword whitelist that did not include 'cat', so this realistic phrase never reached checkAgentAction() at all even though the check's own regex correctly matched it. Fixed by adding 'cat' to the whitelist.",
    files: [{ path: "server/agent-tools/shell-runner.ts", content: tool("run_shell", "cat ~/.ssh/id_rsa") }],
  },
  {
    id: "bash.credential.aws-creds-positive-01",
    ...
    kind: "positive",
    source: "benchmark-new",
    description: "FIXED alongside bash.credential.ssh-key-read above (same isRelevantValue() gap, same fix): an agent tool's shell handler reads the AWS credentials file.",
    files: [{ path: "server/agent-tools/shell-runner.ts", content: tool("run_shell", "cat ~/.aws/credentials") }],
  },
  {
    id: "cron.persistence.at-boot-positive-01",
    ...
    source: "benchmark-new",
    description:
      "FIXED in the Detection Accuracy Hardening V1 pass: an agent tool's cron handler installs an @reboot persistence entry. Previously NOT_COVERED because isRelevantValue() did not recognize '@reboot' as a relevant trigger. Fixed by adding an explicit '@reboot' alternative to the pre-filter.",
    files: [{ path: "server/agent-tools/cron-runner.ts", content: tool("cron", "@reboot /opt/agent/start.sh") }],
  },
];
